...
def cotizaciones_stats_view(request):
    ...
    anio_prev, mes_prev = _mes_anterior(
        año,
        mes,
    )

    inicio_prev, fin_prev = _rango_mes(
        anio_prev,
        mes_prev,
    )
    ...
    def base_activa(d_inicio, d_fin):
        ...
        for estado in ESTADOS_TERMINADOS:
            query_terminado |= Q(
    ...
    # -------------------------------------------------
    #
    # Se consideran por FECHA DE CREACIÓN de la cita,
    # no por fecha_hora_cita.
    citas_digitales = (
        Cita.objects
        .filter(
    ...
    citas_con_cotizacion = (
        ...
        .filter(asistencia=True)
        .filter(
            cliente__expediente_digital__id_cotizacion__gt=""
        )
        .count()
    )

    efectividad = _porcentaje(
        citas_con_cotizacion,
        citas_realizadas,
    )
