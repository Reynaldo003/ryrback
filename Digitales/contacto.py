...
def iniciar_llamada_whatsapp(
    ...
    url = f"https://graph.facebook.com/{GRAPH_VERSION}/{phone_number_id}/calls"

    headers = _json_headers(cfg)

    if not sdp_offer:
        raise ValueError(
            "Calling API está habilitado, pero falta implementar WebRTC/SIP. "
            "Para iniciar una llamada real necesitas enviar action='connect' con session.sdp_type='offer' y session.sdp."
        )
    ...
    if r.status_code >= 400:
        raise RuntimeError(
            ...
            f"numero_asesor={numero_asesor} | phone_number_id={phone_number_id}"
        )
...
def _block_users_api(
    *,
    ...
    url = f"https://graph.facebook.com/{GRAPH_VERSION}/{phone_number_id}/block_users"
    ...
    if response.status_code >= 400:
        raise MetaAPIError(
            status_code=response.status_code,
            error_body=data,
            retryable=_es_error_meta_reintentable(response.status_code, data),
            attempts=1,
...
def bloquear_usuario_whatsapp(to: str, numero_asesor: str) -> dict:
...
def desbloquear_usuario_whatsapp(to: str, numero_asesor: str) -> dict:
    return _block_users_api(
        to=to,
        numero_asesor=numero_asesor,
        method="DELETE",
