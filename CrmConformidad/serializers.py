...
def limpiar_agencias(valor):
    agencias = [
        agencia.strip()
        for agencia in str(valor or "").split("|")
        if agencia.strip()
    ]

    agencias_invalidas = [
        agencia for agencia in agencias
        if agencia not in DEALERS_VALIDOS
    ]

    if agencias_invalidas:
        raise serializers.ValidationError(
            f"Agencia(s) inválida(s): {', '.join(agencias_invalidas)}"
        )
...
class CasoSerializer(serializers.Serializer):
    ...
    def create(self, validated):
        ...
        return ExpedienteConformidad.objects.create(
            cliente=cliente,
    ...
    def update(self, instance, validated):
        ...
        instance.obs_contacto_cierre = validated.get(
            "obs_contacto_cierre",
            instance.obs_contacto_cierre,
        )
        instance.fecha_contacto_cierre = validated.get(
            "fecha_contacto_cierre",
            instance.fecha_contacto_cierre,
        )
...
class UsuarioRegisterSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        ...
        if not rol_empleado:
            raise serializers.ValidationError("No existe el rol 'Empleado'.")
